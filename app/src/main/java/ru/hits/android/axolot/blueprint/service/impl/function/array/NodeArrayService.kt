...
class NodeArrayService(private val nodeHandlerService: NodeHandlerService) : NodeDependencyService {
    ...
    override fun invoke(node: Node, context: InterpreterContext): Variable {
        if (node is NodeArrayFindElement) {
            val array =
                nodeHandlerService.invoke(node.dependencies[NodeArrayFindElement.ARRAY]!!, context)
                    .getArray()!!
            val element = nodeHandlerService.invoke(
                node.dependencies[NodeArrayFindElement.ELEMENT]!!,
                context
            ).value
            ...
            val array =
                nodeHandlerService.invoke(node.dependencies[NodeArrayGetElement.ARRAY]!!, context)
                    .getArray()!!
            val index = nodeHandlerService.invoke(
                node.dependencies[NodeArrayGetElement.INDEX]!!,
                context
            )[Type.INT]!!
            ...
            val array = nodeHandlerService.invoke(node.dependencies[NodeArraySize.ARRAY]!!, context)
                .getArray()!!
